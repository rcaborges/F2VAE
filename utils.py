...
def plot_lines(data, labels):
    ...
    plt.plot(range(len(data[0,:]))[:lmt], data[0,:][:lmt], linestyle='dotted', linewidth=2, color='orange',label=labels[0])
    plt.plot(range(len(data[1,:]))[:lmt], data[1,:][:lmt], linestyle='--', linewidth=2, color='black',label=labels[1])
    plt.plot(range(len(data[2,:]))[:lmt], data[2,:][:lmt], linestyle='dashdot', color='blue', linewidth=2, label=labels[2])
